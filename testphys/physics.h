...
inline void ApplyImpulse(RigidBody *rb,const float3 &r, const float3& impulse)  // r is impact point positionally relative to rb's origin but in 'world' orientation
{
	...
	rb->spin = mul(rb->Iinv , rb->rotation); // recompute auxilary variable spin
...
public:
	...
	void Iter()
	{
		...
		if (rb0)
		{
			...
			rb0->spin = mul(rb0->Iinv , rb0->rotation); // recompute auxilary variable spin
		}
		if (rb1)
		{
			rb1->rotation += axis*dtorque;  // apply impulse
			rb1->spin = mul(rb1->Iinv , rb1->rotation); // recompute auxilary variable spin
		}
		torque += dtorque;
	}
};
...
inline void createdrive(std::vector<LimitAngular> &Angulars_out, RigidBody *rb0,RigidBody *rb1,float4 target,float maxtorque)
{
...
inline LimitLinear limitlinearaxis(RigidBody *rb0,const float3 &p0,RigidBody *rb1,const float3 &p1,const float3 &axisw,float minforce,float maxforce)
{
...
inline void createnail(std::vector<LimitLinear> &Linears_out,RigidBody *rb0,const float3 &p0,RigidBody *rb1,const float3 &p1)
{
...
inline void createangularlimitsW(std::vector<LimitAngular> &Angulars_out, RigidBody *rb0, const float4 &jb0, RigidBody *rb1, const float4 &jf1, const float3& _jointlimitmin, const float3& _jointlimitmax)
{
...
inline void createangularlimits(std::vector<LimitAngular> &Angulars_out, RigidBody *rb0, RigidBody *rb1, const float4 &_jointframe, const float3& _jointlimitmin, const float3& _jointlimitmax)
{
...
inline LimitAngular conelimit(RigidBody* rb0,const float3 &n0,RigidBody* rb1,const float3 &n1,float limitangle_degrees)  // a hinge is a cone with 0 limitangle
{
...
inline RigidBody::RigidBody(std::vector<Shape> shapes_, const float3 &_position) : shapes(shapes_), orientation_next(0, 0, 0, 1), orientation_old(0, 0, 0, 1), orientation_start(0, 0, 0, 1)
{
...
inline void rbinitvelocity(RigidBody *rb)
{
	...
	rb->spin = mul(rb->Iinv, rb->rotation);
}
